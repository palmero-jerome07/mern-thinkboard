...
export async function getAllNotes(_, res) {
  // if variable is not used, make sure it will be replaced by "_"
  try {
...
export async function getNoteById(req, res) {
  try {
    ...
    res
      .status(200)
      .json({ message: "You just fetched the note", note: noteById });
  } catch (error) {
...
export async function deleteNote(req, res) {
  try {
    ...
    res
      .status(200)
      .json({ message: "Note deleted successfully", deletedNote: deletedNote });
  } catch (error) {
...
export async function addReply(req, res) {
  try {
    ...
    note.replies.push({ content: content.trim() });
    const savedNote = await note.save();

    res.status(201).json({ note: savedNote });
  } catch (error) {
    console.error("Error in addReply controller", error);
    res.status(500).json({ message: "Internal server error" });
  }
}
